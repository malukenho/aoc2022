    ...
    private val monkeys = input.split("\n\n").map {
        ...
        it.lines().forEach { line ->
            when {
                ...
                line.startsWith("  Test: divisible by ")          -> divisibleBy =
                    line.replace("  Test: divisible by ", "").toLong()

                line.startsWith("    If true: throw to monkey ")  -> ifTrue =
                    line.replace("    If true: throw to monkey ", "").toInt()

                line.startsWith("    If false: throw to monkey ") -> ifFalse =
                    line.replace("    If false: throw to monkey ", "").toInt()

                line.startsWith("Monkey ")                        -> monkeyID = line.split(' ').last().trim(':').toInt()
                line.startsWith("  Starting items: ")             -> items =
                    line.replace("  Starting items: ", "").split(", ").map { Item(it.toLong()) }
            }
        }
